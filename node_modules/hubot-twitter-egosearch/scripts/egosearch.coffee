...
createQuery = (robot, path, msg, q, user) ->
...
searchQueryAsync = (id, query) ->
  new Promise (resolve, reject) ->
    client.get query.path, {user_id: query.q, q: query.q, count: maxTweets, since_id: query.since_id}, (error, tweets, response) ->
      if error
        reject error
...
class Query
  constructor: (path, q, since_id, user) ->
    @path = path
    @q = q
    @since_id = since_id
...
module.exports = (robot) ->
  ...
  robot.respond /egosearch (?:user) (.+)/i, (msg) ->
    createQuery robot, 'statuses/user_timeline', msg, msg.match[1], msg.message.user

  robot.respond /egosearch (?:add|new|start) (.+)/i, (msg) ->
    createQuery robot, 'search/tweets', msg, msg.match[1], msg.message.user
