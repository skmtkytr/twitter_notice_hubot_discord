...
persistQuery = function(robot, id, query) {
  queries[id] = query;
  return robot.brain.data.egosearch[id] = query.serialize();
};

createQuery = function(robot, path, msg, q, user) {
  var id, query;
  query = new Query(path, q, null, user);
...
Query = (function() {
  function Query(path, q, since_id, user) {
    var clonedUser, k, v;
    this.path = path;
    this.q = q;
  ...
  Query.prototype.serialize = function() {
    return [this.path, this.q, this.since_id, this.user];
  };
...
module.exports = function(robot) {
  ...
  robot.respond(/egosearch (?:user) (.+)/i, function(msg) {
    return createQuery(robot, 'statuses/user_timeline', msg, msg.match[1], msg.message.user);
  });
  robot.respond(/egosearch (?:add|new|start) (.+)/i, function(msg) {
    return createQuery(robot, 'search/tweets', msg, msg.match[1], msg.message.user);
  });
